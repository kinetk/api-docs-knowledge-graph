...
export default function AgentView() {
  ...
  const [result, setResult] = useState<{ signals: GrowthSignals; gen: GrowthGen } | null>(null);
  ...
  const run = useCallback(async (product: string) => {
    ...
    const finish = (signals: GrowthSignals, gen: GrowthGen) => {
      ...
      push({ type: "metric", label: "narratives", value: `${signals.narratives.length}` });
      push({ type: "metric", label: "selling hooks", value: `${gen.hooks.length}` });
      push({ type: "metric", label: "proof posts", value: `${signals.records.length}` });
      push({ type: "thought", text: "Gemini authored the launch plan — grounded in the live insight signal." });
    ...
    try {
      ...
      push({
        ...
        args: { kind: "insights", query: product, limit: 3000, window: "all", includeSignals: true },
      });
      ...
      for (let i = 0; i < MAX_POLLS; i++) {
        ...
        if (myRun !== runIdRef.current) return;
        const res = await postAgent<PollResp>({ action: "poll", product, jobId: start.jobId });
        if (res.phase === "ready") {
          push({ type: "tool_call", id: "result", tool: "get_context_job_result", args: { jobId: start.jobId, verbose: true } });
          push({ type: "tool_result", id: "result", ok: true, summary: "insights result returned" });
          ready = true;
          break;
        }
        const sid = `status-${i}`;
        push({ type: "tool_call", id: sid, tool: "get_context_job_status", args: { jobId: start.jobId } });
        push({ type: "tool_result", id: sid, ok: true, summary: `${res.status ?? "running"} · ${(i + 1) * 10}s elapsed` });
      }
      if (!ready) throw new Error("Timed out waiting for the insights job");

      push({ type: "synthesize", model: SYNTH_MODEL });
      const syn = await postAgent<SynthResp>({ action: "synthesize", product, jobId: start.jobId });
      ...
      if (myRun === runIdRef.current) {
        push({ type: "error", message: e instanceof Error ? e.message : "Run failed" });
        setRunning(false);
      }
    }
  }, []);
      ...
      <div className="mx-auto max-w-3xl">
        {!started && (
          <div className="mb-8 text-center">
            ...
            Describe your product in one sentence. KINETK analyzes live social signals and generates an evidence-backed launch and growth plan.
            </p>
          </div>
        )}
        <ProductInput onRun={run} running={running} />
      </div>
            ...
            {result ? (
              ...
              <div className="grid h-[420px] place-items-center rounded-xl border border-dashed border-white/10 lg:h-[680px]">
                <p className="text-sm text-white/40">
                  {running ? "Agent is working — Gemini authoring the launch plan…" : "Awaiting result…"}
                </p>
              </div>
            )}
          </div>
